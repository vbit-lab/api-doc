    ...
    def do_login(self):
        ...
        def on_cmd_time_resp(_, code, data):
            ...
            self.__send_command('Login',
                               {'UserName': self.user_name, 'UserCred': self.api_key, 'DeviceInfo': 'vbit-api-py sample'
                                }, on_cmd_login_resp)
...
class VbitMarketWebsocket:
    ...
    def REQ_GetCompositeIndex(self, cb):
        self.__send_command('GetCompositeIndex', {'vp': 30}, cb)

    def REQ_GetAssetD(self, cb):
        self.__send_command('GetAssetD', {'vp': 30}, cb)

    def REQ_GetAssetEx(self, cb):
        self.__send_command('GetAssetEx', {'vp': 30}, cb)
