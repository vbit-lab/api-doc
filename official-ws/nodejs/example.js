const Vbit = require('./lib/lib')

const test_market_url = 'wss://cdn01.mcdzcloud.com/v1/market';
...
market.init({ws_url:test_market_url}, ()=>{
    ...
    market.request('GetAssetD',{vp: 30},(ret)=>{
        console.log(`[market.GetAssetD]<< ${JSON.stringify(ret)}`)
        if(ret.code==0 && ret.data.length > 0) {
            var sym = ret.data[0].Sym; // example: BTC1903 or BTC/BTC etc.
